...
class AgentsList extends CBitrixComponent implements Controllerable, Errorable
{
    ...
    final public function executeComponent(): void
    {
        ...
        $category = 'favagents';
        $name = "Agents_ID";
         $this->arResult['favagents'] = CUserOptions::GetOption($category, $name);
        /*
    ...
    private function getFieldListValue(array $arHlblock, string $fieldName): array
    {
        ...
        if ($fieldID) {
            $dat = CUserFieldEnum::GetList(array(), array(
    ...
    private function getAgents(string $entity, array $arTypeAgents): array
    {
        ...
        $nav->allowAllRecords(true)
            ->setPageSize($this->arParams['PAGE_COUNT']) //Нужно передать параметр Количество элементов из мввсива $this->arParams
            ->initFromUri();
    ...
    public function clickStarAction($agentID)
    {
        ...
        $userOption = CUserOptions::GetOption("favagents", "Agents_ID");
        ...
        if ($userOption != "") {
            ...
            if (in_array($agentID, $userOption)) {
                $userOption = array_diff($userOption, array($agentID));
        ...
        if (CUserOptions::SetOption("favagents", "Agents_ID", $value)) {
